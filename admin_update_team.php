...
if (!$id_equipe) {
    echo json_encode(["success" => false, "message" => "ID équipe manquant"]);
    exit;
}
...
try {
    ...
    // Vérifier que l'équipe appartient à l'admin
    $check = $conn->prepare("SELECT id_equipe FROM equipe WHERE id_equipe = :id_equipe AND id_admin = :id_admin");
    $check->bindParam(':id_equipe', $id_equipe);
    $check->bindParam(':id_admin', $id_admin);
    $check->execute();

    if (!$check->fetch()) {
        echo json_encode(["success" => false, "message" => "Vous ne pouvez pas modifier cette équipe"]);
        exit;
    }
